...
\textbf{案例研究：架构如何防御真实越狱攻击。}我们在 \texttt{action\_space\_benchmark.py} 中实现了三个基于真实已知越狱模式的结构化场景：\textbf{(1) DAN (Do Anything Now)}——攻击者通过角色重写提示注入无限制人格，继而请求有害内容。在超图模型中，Step~1 获取 \texttt{jailbreak:role\_override}，Step~2 获取 \texttt{jailbreak:harmful\_gen}，两者合取触发 \texttt{forbidden:dan\_jailbreak\_success}。解耦内核在 Step~2 的 Judge 评估中检测到合取能力依赖并 Deny，阻断攻击链。\textbf{(2) Grandma exploit}——情感操纵类越狱，攻击者利用祖母叙事建立共情后请求危险信息，同样在 Step~2 被合取能力围栏阻断。\textbf{(3) Multi-turn crescendo}——多轮逐渐升级攻击，从良性化学问题起步，逐步过渡到有害合成请求。每一步\emph{单独}均为良性（Theorem~3.1），但三步合取触发 \texttt{forbidden:crescendo\_jailbreak}，在 Step~3 被内核拦截。
...
完整策略示例见仓库 \texttt{rfc\_contracts/example\_policies/} 目录（共 5 个策略文件覆盖 PII 过滤、提示注入防御、有害内容分类、多 Agent 能力围栏和交互预算控制）。
