...
**Status:** Draft — 规范冻结前收口稿；已增补 **§12.4** 实证评估与论文复现映射（信息性，不改动 §5–§11 MUST 语义）。
...
本 RFC 定义与基础大语言模型（LLM）**解耦**的 **Safety Kernel**：将基础模型视为 Ring-3 **非可信**用户态生成进程，在 Ring-0/1 以**可审计契约**强制执行输入净化、离散时间 DCBF 监测、DSL→自动机策略、多验证器裁决、候选集联合投影及 Graceful Degradation。规范要求每步生成在**有界时间**内完成，默认在冲突、不可行、超时或审计失败时走 **fail-safe**（以 deny/降级为主），并通过 **Audit / Evidence Chain** 绑定 `trace_id` 与规则/系数版本。参考实现可提供**可复现实证协议**与脚本索引（**信息性**，见第 **12.4** 节），用于论文级指标与回归，**不**升格为规范安全义务。

**非目标**不在摘要展开，见第 1 节；**威胁模型**见第 2 节；**全局安全不变量**见第 8 节（含 I6 提交顺序）；**故障分类**见第 9 节；**内核测试与故障注入**见第 12.1–12.3 节；**实证评估与论文复现映射**见第 **12.4** 节（信息性）；**审计与配置**见第 11、13 节。

**关键词：** decoupled safety、control barrier function、runtime policy automaton、verifier ensemble、constraint projection、evidence chain、hard latency budget、empirical evaluation protocol
...
6. **Ring-4 实证编排**（论文 harness、版本化提示集、可选 **HTTP / LLM-as-judge** 用于 RSR、良性误拒代理、抽取 \(F_1\) 等）**仅用于测量与复现**，**不得（MUST NOT）**在文档、制品元数据或部署叙事中被表述为 **Ring-0 `Judge Ensemble`（§5）** 的规范替代品或生产链上的「最终语义仲裁」。与第 4 条一致：规范意义上的放行语义仍须由 **多 Verifier Ensemble + 默认 deny** 承担；外置裁判的 JSON 标签**不**满足 §11 对 `ensemble` 审计字段的 MUST。
...
| **Ring-4** | 论文实证 harness、版本化数据集、可选 HTTP/LLM 裁判（如 Track A）、结果导出脚本                      | **无**策略写权限；**不**承担 I2/I6 生产放行链；**不**等同 §5 Judge Ensemble |
...
- **Ring-4（实证编排）**：编排上游 API、内核脚本与测量裁判；产出 JSONL/summary 等**研究工件**；**不得**冒充 Ring-0 裁决或审计源。
...
**实现注记（信息性）：** 仓库内 `decoupled-safety-kernel/scripts/e2e_full_chain.py` 与 `e2e_ring3_json`（Rust）实现的是 **截面子链**——Ring-3 上游 top-k / logprobs（及可选独立 DCBF v2 探针）经 JSON IPC 进入 Ring-0 投影与 I6 审计——**未**在本图中展开 Gateway 的完整前置路径或全部 Policy/Ensemble 组合。完整安全拓扑仍以本图与 **§5** 类型契约为准；与论文指标对齐的脚本索引见 **§12.4**。
...
### 12.4 实证评估与论文复现映射（信息性）

本节为 **信息性（informative）**：描述当前 **`decoupled-safety-kernel`** 仓库中与论文主张、Gap 闭环及 Track A 主表对齐的**可执行制品**与**指标字段**。**不**新增、**不**弱化 **§5–§11** 对 Safety Kernel 的 MUST/SHOULD 语义；**不**将 Ring-4 测量裁判等同于 **§5 Judge Ensemble**（见 **§1 非目标**第 6 条、**§3.1**）。

**详细操作说明（环境变量、双终端启动 judge、数据集路径）**见仓库内：

- `docs/paper/one_pager.md` — 论文素材与 **代码索引 / 指标表 / 离线·在线命令** 的 SSOT 对照；  
- `decoupled-safety-kernel/paper_eval/README.md` — **Track A** harness、`judge_mode`、`TRACKA_JUDGE_URL`、LaTeX 导出等。

仓库根目录 `env.example` 描述上游（如 DeepSeek 直连）与 OpenRouter 裁判（可选 SOCKS5）等密钥占位；**不得**将示例文件提交为含真实密钥的 `env`。

| 实验主张 / Gap | 主要脚本或二进制（相对 `decoupled-safety-kernel/`） | 关键指标或输出字段 |
| -------------- | --------------------------------------------------- | ------------------- |
| 定理 3.1 式**组合攻击**、隐式对齐陷阱（文本域） | `src_eval_benchmark/compositional_hypergraph_attack.py`；`scripts/live_fire_benchmark.py` | `composability_gain`；场景级 `baseline_forbidden` / `defended_forbidden` |
| **Gap 2** 控制层活性、投影过度阻断、QP 预算 | `src_eval_benchmark/utility_benchmark.py` | `projection_over_blocking_rate`、`qp_timeout_rate`、`mean_qp_elapsed_us`、`mean_rust_total_us` 等 |
| **Gap 4** 动作空间、工具调用合取 | `src_eval_benchmark/action_space_benchmark.py` | `action_composability_gain`、`steps_before_block`、场景级阻断步 |
| **Gap 1** + 全链时延、DCBF v1 vs v2 | `scripts/e2e_full_chain.py`（`--dcbf-v2` 等）；Ring-0 `src_kernel/src/bin/e2e_ring3_json.rs`（`cargo build --bin e2e_ring3_json`） | 逐步 `dcbf_margin`、`profile.rust_total_us`、`profile.qp_elapsed_us`、`profile.cache_hit`、`profile.qp_budget_exceeded` 等 |
| **Track A（Ring-4）** 主表：RSR、良性误拒代理、抽取 \(F_1\) | `scripts/run_trackA_harness.py`；`src_paper_eval/judge_service.py`（可选 `--backend chat`）；`scripts/export_trackA_table_tex.py` | `harmful_rsr`、`benign_refusal_fpr`、`mean_extract_f1` / `max_extract_f1`；`variant_id` 内 `dcbf_mode`、`judge_mode`、`action_space_guard`；Ring-0 聚合如 `mean_rust_total_us_last_step`、`ring0_cache_hit_rate` |
| DCBF v2 探针单元回归 | `tests/test_proxy_ensemble.py`（pytest） | 探针与 margin 逻辑一致性 |
| **Gap 3** IPC 帧协议（Python 侧自测） | `scripts/mmap_adapter.py` | 帧布局与 happens-before 自检输出 |

**规范对齐提示：** 论文或附录中引用的实现路径 **应以本表与上述两份 Markdown 为准**；若正文仍出现已迁移的旧仓库名或旧协议名，应在出版前改为 `decoupled-safety-kernel` 与当前脚本名，以免与 RFC 及 CI 制品不一致。
...
- **r2 正文 + 评估附录（信息性）**：增补 **§3.1 Ring-4**、**§3.2** 与 **§3.3** 下 `e2e` 截面子链注记、**§1 非目标**第 6 条（Ring-4 裁判边界）、以及 **§12.4** 实证映射表与对外 Markdown 引用。上述增补 **不**改变 §2–§11 中安全不变量、接口 MUST 与故障语义的解释；实现者若仅对齐「冻结内核契约」，可忽略 §12.4 而不影响合规性判定（但论文复现与 CI 基准仍 **建议** 以 §12.4 索引为准）。  
...
**论文—实现对照（非规范替代品）：** 仓库根目录 `docs/paper/one_pager.md`（方法、威胁模型、指标、离线/在线命令索引）与 `decoupled-safety-kernel/paper_eval/README.md`（Track A harness、裁判模式、LaTeX 导出）用于与正文实验部分、表项字段对齐；若与 **本 RFC 正文**冲突，**以本 RFC 为准**；若与 **实际脚本行为**冲突，应优先修正实现或更新 §12.4 索引表并保留审计式变更记录。
...
- 更大规模社区基准（如 JBB-Behaviors、in-the-wild 集）与 Track A 主表的扩展切片：在 **不改变 Ring-0 MUST** 的前提下，通过 **§12.4** 所列 harness 迭代数据集版本与裁判协议，并在论文元数据中固定哈希与种子。
...
Decoupled Safety Kernel **v0.2-r2** 在 v0.2 **正文**之上完成冻结前收口：多实现可对齐的类型闭合、fault 粒度、候选–裁决索引语义与数学/渲染可审计性；并收录 **§12.4** 与当前 `decoupled-safety-kernel` 实现一致的**信息性**实证索引，供论文复现与回归对照。实现应 **以本文件（v0.2-r2）为单一权威**（SSOT）进行对齐与评审。
